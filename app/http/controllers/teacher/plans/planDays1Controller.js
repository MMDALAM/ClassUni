...
class planDays1Controller extends controller  {
    async showPlanDays1Form(req , res){
        const teachersAuth = await this.Teachers();
        const lessonses = await this.Lessons();
        const classCode = await this.Classes();
        const terms = await this.terms();
        const fields = await this.fields();

        res.render('./teacher/plan/planDays1' , {teachersAuth , lessonses , classCode , terms ,fields});
    }

    async planDays1Proccess(req , res , next) {
    ...
    async Classes(req , res){
        const classes = await Class.find({ Reserve: false , days : "شنبه" })

        const classCode = [];
        classes.forEach((el) =>{
            classCode.push(el.classId);
        })
        return classCode;
    }
    ...
    async plan(req , res , next){
        ...
        if (req.body.classId2){
            if(currentClass2 === null){
                ...
                return res.redirect('/teacher/planDays1');
            }}
            }}
...
module.exports = new planDays1Controller();