...
class planTuesdayController extends controller  {
    async showPlanTuesdayForm(req , res){
        const repTuesday = await this.planTuesday();

        res.render('./auth/daysPlan/planTuesday' , {repTuesday});
    }
    
    async planTuesday (req , res){
        const rep = await  Plan.find({ days : "سه شنبه" }).sort({ classId : 1 }).sort({ field : 1 }).sort({ classTime1 : 1 }).sort({ termCode : 1 });
...
module.exports = new planTuesdayController();